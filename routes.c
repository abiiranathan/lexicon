...
static inline void send_json_error(connection_t* conn, const char* msg) {
    char json_str[128];
    snprintf(json_str, sizeof(json_str) - 1, "\"{\"error\": \"%s\"}\"", msg);
...
void pdf_search(connection_t* conn) {
    ...
    if (!pgpool_prepare(dbconn, "pdf_query", search_query, 1, NULL, TIMEOUT_MS)) {
        send_json_error(conn, "Failed to prepare statement for search");
        return;
    }
    ...
    if (!res) {
        send_json_error(conn, "Failed to execute prepared statement");
        return;
    }
    defer({ PQclear(res); });
    ...
    if (PQresultStatus(res) != PGRES_TUPLES_OK) {
        send_json_error(conn, "Database query execution failed");
        return;
    }
